...
// import { HttpRequest } from '@salesforce/core/lib/http-client/types';
  ...
  public async request<T extends nock.Body>(
    ...
    if (this.mockDir) {
      ...
      switch (method) {
        ...
        case 'DELETE':
          for (const response of responses) {
            scope.delete(url).reply(200, response);
          }
          break;
      }
    }
    ...
    // TEMP HACK: JSForce overwrites the access token on the request
    this.connection.accessToken = '';

    const options = { retry: { maxRetries: 3 } };

    switch (method) {
      case 'GET':
        return this.connection.request<T>({ method: 'GET', url, headers }, options);
      case 'POST':
        if (!body) {
          throw SfError.create({
            name: 'InvalidBody',
        ...
        // @ts-expect-error: Nock is causing a type error here
        return this.connection.request<T>({ method: 'POST', url, headers, body }, options);
      case 'DELETE':
