...
export class ScriptAgentPublisher {
  ...
  public async publishAgentJson(): Promise<PublishAgent> {
    ...
    // Step 1: Deploy the AuthoringBundle without target
    ...
    // Step 2: Use JWT token only for the publish API call, then restore connection
    ...
    if (response.botId && response.botVersionId) {
      ...
      // Step 3: Retrieve the new Agent metadata that's in the org
      if (!this.skipRetrieve) {
        await this.retrieveAgentMetadata(actualVersionName);
      }

      // Step 4: Deploy the authoring bundle again with the actual version as target
